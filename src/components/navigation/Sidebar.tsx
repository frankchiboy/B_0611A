        ...
        {expanded && currentProject && (
          <div className="mb-6">
            <div className="relative">
              ...
              {/* 專案列表下拉選單 */}
              {showProjectList && (
                <div className="absolute top-full left-0 right-0 bg-white/95 backdrop-blur-sm rounded-lg shadow-xl border border-slate-200 py-2 z-50 max-h-64 overflow-y-auto">
                  <div className="px-3 py-2 border-b border-slate-100">
                    <h4 className="text-xs font-medium text-slate-600 uppercase">選擇專案</h4>
                  </div>
                  ...
                  {projects.map(project => (
                    <button
                      ...
                      className={`w-full px-3 py-2 text-left hover:bg-slate-100/80 transition-colors ${
                        currentProject.id === project.id ? 'bg-teal-100/80 text-teal-800' : 'text-slate-800'
                      }`}
                    >
                      <div className="flex items-center">
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium truncate">{project.name}</div>
                          <div className="text-xs text-slate-600">{project.progress}% 完成</div>
                        </div>
                  ...
                  <div className="border-t border-slate-100 mt-2 pt-2">
                    <button
                      onClick={handleCreateNewProject}
                      className="w-full px-3 py-2 text-left text-sm text-teal-700 hover:bg-teal-100/80 transition-colors flex items-center font-medium"
                    >
